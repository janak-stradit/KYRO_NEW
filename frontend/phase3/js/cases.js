...
const Cases = {
    ...
    generateExport(data, format) {
        ...
        let caseLabel = String(data.caseId || 'CASE');
        if (!caseLabel.toUpperCase().startsWith('CASE-')) {
            caseLabel = `CASE-${caseLabel.substring(0, 6).toUpperCase()}`;
        }
        
        let custLabel = String(data.customerId || 'CUSTOMER');
        if (!custLabel.toUpperCase().startsWith('CUST-') && custLabel.length > 8) {
            custLabel = `CUST-${custLabel.substring(0, 6).toUpperCase()}`;
        }

        const safeCaseLabel = caseLabel.replace(/[^a-zA-Z0-9_-]/g, '');
        const safeCustLabel = custLabel.replace(/[^a-zA-Z0-9_-]/g, '');

        const filename = `KYRO_Case_${safeCaseLabel}_${safeCustLabel}`;
        
        console.log(`Generating case export for ${data.caseId} -> ${filename}.${fmt}`);
        ...
        switch(fmt) {
            ...
            case 'excel':
            case 'xlsx':
                const excelContent = this.generateCSVContent(data);
                this.downloadFile(`${filename}.xlsx`, 'application/vnd.ms-excel', excelContent);
                break;
            ...
            case 'json':
                ...
                this.downloadFile(`${filename}.json`, 'application/json;charset=utf-8;', jsonContent);
                break;
    ...
    generateHTMLPDF(data, filename) {
        ...
        if (!jsPDFConstructor) {
            console.warn("jsPDF library not available, producing text report download");
            const reportContent = `KYRO CASE EXPORT REPORT\n=======================\nCase ID: ${data.caseId}\nCustomer ID: ${data.customerId}\nExport Date: ${new Date(data.exportDate).toLocaleString()}\nStatus: ${data.caseDetails?.status || 'OPEN'}\nRisk Level: ${data.caseDetails?.riskLevel || 'HIGH'}\nPriority: ${data.caseDetails?.priority || 'URGENT'}\nTrigger Type: ${data.caseDetails?.triggerType || 'BEHAVIOR BASED'}\nCreated At: ${data.caseDetails?.createdAt || 'N/A'}\nAssigned To: ${data.caseDetails?.assignedTo || 'Unassigned'}\n\nIncluded Sections: ${(data.sections || []).join(', ')}\n\nConfidential report generated by KYRO AML Risk Assessment System.`;
            this.downloadFile(`${filename}.txt`, 'text/plain;charset=utf-8;', reportContent);
            return;
        }
